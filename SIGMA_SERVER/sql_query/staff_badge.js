...
"   WHEN DATE_ADD(DateBegin, INTERVAL g.period YEAR) > now() THEN CONCAT('<span class=\"badge text-bg-success\">', g.name, ' ', IFNULL(so.SerNo,''), '</span>',' ') "+
"   WHEN DATE_ADD(DateBegin, INTERVAL g.period YEAR) < now() THEN CONCAT('<span class=\"badge text-bg-danger\">', g.name, ' ', IFNULL(so.SerNo,''), '</span>',' ') "+
